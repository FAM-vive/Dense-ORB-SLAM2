...
void KeyFrame::UpdateConnections() {
  ...
  //===============1==================================
  map<KeyFrame *, int>
      KFcounter;  // 关键帧-权重，权重为其它关键帧与当前关键帧共视3d点的个数
  ...
  for (vector<MapPoint *>::iterator vit = vpMP.begin(), vend = vpMP.end();
    ...
    for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      KFcounter
          [mit->first]++;  //标签对应的整数+1，没有则创建mit，第一个数为关键帧，第二个数为它们具有共识点数
    }
  }
          ...
          this);  // 让与自己有联系的MapPoint删除与自己的联系
  {
    ...
    while (!mspChildrens.empty()) {
      ...
      for (set<KeyFrame *>::iterator sit = mspChildrens.begin(),
        ...
        for (size_t i = 0, iend = vpConnected.size(); i < iend;
             i++)  //遍历每一个子关键帧的每个共视帧
        {
               ...
               spcit != spcend; spcit++)  //遍历候选父关键帧
          {
            ...
            // 举例：B-->A（B的父节点是A） C-->B（C的父节点是B）
            // D--C（D与C相连） E--C（E与C相连） F--C（F与C相连）
            // D-->A（D的父节点是A） E-->A（E的父节点是A）
            ...
            //           C与E间的权重大，因此D为pP，这种说法有错误
            if (vpConnected[i]->mnId ==
                (*spcit)
                    ->mnId)  //如果子帧的共视帧在父帧候选集中，目前就一个A，获取其为父帧
            {
              int w = pKF->GetWeight(vpConnected[i]);
              if (w > max) {
                pC = pKF;  //该挂点的子节点
                pP = vpConnected
                    [i];  //在这里就是A，并且A点是与该挂点的子节点的该共视帧
                max = w;
                bContinue = true;
              }
            }
      ...
      if (bContinue) {
        ...
        // 该子节点处理完毕
        mspChildrens.erase(
            pC);  //在子节点中去掉，返回while重新进行，直到continue为false
      } else
        break;
    }

    // If a children has no covisibility links with any parent candidate, assign
    // to the original parent of this KF 如果还有子节点没有找到新的父节点
    ...
    mTcp =
        Tcw *
        mpParent->GetPoseInverse();  // mTcp * 父节点位姿 = 该被剔除关键帧的位姿
    mbBad = true;
  }
