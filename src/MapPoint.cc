...
MapPoint::MapPoint(const cv::Mat& Pos, Map* pMap, Frame* pFrame,
  ...
  mNormalVector = mNormalVector /
                  cv::norm(mNormalVector);  // 世界坐标系下相机到3D点的单位向量
...
void MapPoint::UpdateNormalAndDepth() {
  ...
  for (map<KeyFrame*, size_t>::iterator mit = observations.begin(),
    ...
    normal =
        ...
        normali /
            cv::norm(
                normali);  // 对所有关键帧对该点的观测方向归一化为单位向量进行求和
    n++;
  }
  ...
  cv::Mat PC =
      Pos -
      pRefKF
          ->GetCameraCenter();  // 参考关键帧相机指向3D点的向量（在世界坐标系下的表示）
...

  {
    ...
    mfMinDistance =
        mfMaxDistance /
        pRefKF->mvScaleFactors[nLevels - 1];  // 观测到该点的距离上限
