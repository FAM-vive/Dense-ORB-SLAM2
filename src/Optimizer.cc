...
void Optimizer::BundleAdjustment(const vector<KeyFrame*>& vpKFs,
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    vPoint->setEstimate(Converter::toVector3d(
        pMP->GetWorldPos()));  // Eigen::Matrix<double,3,1>
    ...
    const map<KeyFrame*, size_t> observations =
        pMP->GetObservations();  //查找看到该关键点的所有帧，格式1,1  .2,1
    ...
    for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin();
      ...
      const cv::KeyPoint& kpUn =
          pKF->mvKeysUn[mit->second];  //该mp在当前关键帧的特征点

      // 单目或RGBD相机
      if (pKF->mvuRight[mit->second] <
          0)  //只有双目才有值，其余都是-1，属于Frame成员变量
      {
        Eigen::Matrix<double, 2, 1> obs;
        obs << kpUn.pt.x, kpUn.pt.y;

        g2o::EdgeSE3ProjectXYZ* e = new g2o::EdgeSE3ProjectXYZ();
        //建立每个mp与帧的边 BaseBinaryEdge<2, Vector2d, VertexSBAPointXYZ,
        //VertexSE3Expmap> 2，边的类型，两个节点的类型 g2o如下定义，所以0
        // 1不能对换 不然报错
        //    const VertexSE3Expmap* v1 = static_cast<const
        //    VertexSE3Expmap*>(_vertices[1]); const VertexSBAPointXYZ* v2 =
        //    static_cast<const VertexSBAPointXYZ*>(_vertices[0]);
        //_error = obs-cam_project(v1->estimate().map(v2->estimate()));
        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                            optimizer.vertex(id)));  // VertexSBAPointXYZ
        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                            optimizer.vertex(pKF->mnId)));  // VertexSE3Expmap
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (nLoopKF == 0) {
      ...
      Converter::toCvMat(SE3quat).copyTo(
          pKF->mTcwGBA);               //更新后的位姿放入mTcwGBA
...
int Optimizer::PoseOptimization(Frame* pFrame) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (pMP) {
        ...
        if (pFrame->mvuRight[i] < 0) {
          ...
          //该优化优化的是
          //Tcw,根据给定的相机参数，点的三维坐标，投影到该帧，计算重投影误差
          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(0)));
          e->setMeasurement(obs);
  ...
  // 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）----不知道怎么算的，查表可知
  // 下面两个分别是服从 2和3自由度的卡方分布 0.5的值
  ...
  for (size_t it = 0; it < 4; it++) {
    vSE3->setEstimate(
        Converter::toSE3Quat(pFrame->mTcw));  //每次优化重新确定节点待优化数值
    ...
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
      ...
      const size_t idx = vnIndexEdgeMono
          [i];  //单目中，每个特征点都会连接边，这是每个边的特征点的id
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag,
  ...
  for (int i = 0, iend = vNeighKFs.size(); i < iend; i++) {
    ...
    pKFi->mnBALocalForKF =
        pKF->mnId;  //记录局部优化id，该数为不断变化，数值等于局部化的关键帧的id，该id用于防止重复添加
  ...
  // 步骤7：添加顶点：Pose of Fixed
  // KeyFrame，注意这里调用了vSE3->setFixed(true)。
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
  ...
  vector<g2o::EdgeInverseSim3ProjectXYZ*>
      vpEdges21;  // pKF1对应的MapPoints到pKF2的投影
