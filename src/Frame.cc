...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
  ...
  mvpMapPoints = vector<MapPoint *>(
      N, static_cast<MapPoint *>(NULL));  // NULL强制转化为MapPoint*类型
  mvbOutlier = vector<bool>(N, false);
  ...
  if (mbInitialComputations) {
    ...
    mfGridElementWidthInv = static_cast<float>(FRAME_GRID_COLS) /
                            static_cast<float>(mnMaxX - mnMinX);  // 1/横向个数
...
void Frame::UpdatePoseMatrices() {
  ...
  // mtcw, 即相机坐标系下相机坐标系到世界坐标系间的向量,
  // 向量方向由相机坐标系指向世界坐标系 mOw,
  // 即世界坐标系下世界坐标系到相机坐标系间的向量,
  // 向量方向由世界坐标系指向相机坐标系
}
/**
 * @brief 判断一个点是否在视野内
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit) {
  ...
  const cv::Mat PO = P - mOw;  // RtX1-RtT = p / -RtT=mOw 所以PO是RtX1
                               // 也就是三维点在当前帧坐标下的坐标左乘一个mRwc
  ...
  // Check viewing angle
  // V-D 2) 计算当前视角和平均视角夹角的余弦值, 若小于cos(60),
  // 即夹角大于60度则返回
  ...
  const int nPredictedLevel =
      pMP->PredictScale(dist, this);  // this返回当前类也就是Frame
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float &y,
  ...
  const int nMinCellX = max(
      0, (int)floor(
             (x - mnMinX - r) *
             mfGridElementWidthInv));  // floor  5.4
                                       // 取5，此命令计算x点在哪个格子，跟0取最大
  ...
  for (int ix = nMinCellX; ix <= nMaxCellX; ix++) {
    for (int iy = nMinCellY; iy <= nMaxCellY; iy++) {
      const vector<size_t> vCell =
          mGrid[ix][iy];  //将该格里面的特征点的标号存入vCell
...
bool Frame::PosInGrid(
    const cv::KeyPoint &kp, int &posX,
    int &posY)  //引用，计算点在第几个grid内，判断点是否在grid内
{
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)  //计算图像边界
{
  if (mDistCoef.at<float>(0) != 0.0) {
    ...
    // Undistort corners
    mat = mat.reshape(2);
    cv::undistortPoints(mat, mat, mK, mDistCoef, cv::Mat(),
                        mK);  //计算图中经过畸变计算的二维点位置
    mat = mat.reshape(1);
...
void Frame::ComputeStereoMatches() {
  ...
  const int nRows = mpORBextractorLeft->mvImagePyramid[0]
                        .rows;  // 第一层图像的行数，也就是y，也就是v
  ...
  const float minZ =
      mb;  // NOTE bug
           // mb没有初始化，mb的赋值在构造函数中放在ComputeStereoMatches函数的后面
  const float minD = 0;           // 最小视差, 设置为0即可
  const float maxD = mbf / minZ;  // 最大视差, 对应最小深度 mbf/minZ = mbf/mb =
                                  // mbf/(mbf/fx) = fx (wubo???)

  // For each left keypoint search a match in the right image
  vector<pair<int, int> > vDistIdx;
  vDistIdx.reserve(N);
  // 步骤2：对左目相机每个特征点，通过描述子在右目带状搜索区域找到匹配点,
  // 再通过SAD做亚像素匹配 注意：这里是校正前的mvKeys，而不是校正后的mvKeysUn
  ...
  for (int iL = 0; iL < N; iL++) {
    ...
    if (bestDist < thOrbDist)  // 75
    {
      ...
      // (scaleduL, scaledvL) (scaleduR0, ) 左右特征点貌似目前还不出一同一层
      ...
      const float iniu =
          scaleduR0 + L - w;  //这个地方是否应该是scaleduR0-L-w (wubo???)
  ...
  sort(vDistIdx.begin(),
       vDistIdx.end());  // 根据所有匹配对的SAD偏差进行排序, 距离由小到大
  const float median = vDistIdx[vDistIdx.size() / 2].first;
  const float thDist =
      1.5f * 1.4f * median;  // 计算自适应距离, 大于此距离的匹配对将剔除
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (d > 0) {
      mvDepth[i] = d;                    //得到关键点的深度信息；
      mvuRight[i] = kpU.pt.x - mbf / d;  //得到右图的横向坐标；
    }
  }
}
/**
 * @brief Backprojects a keypoint (if stereo/depth info available) into 3D world
 * coordinates.
 * @param  i 第i个keypoint
 * @return   3D点（相对于世界坐标系）
 */
