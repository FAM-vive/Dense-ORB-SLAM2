 ...
 * @brief 计算单应矩阵
 *
 * 假设场景为平面情况下通过前两帧求取Homography矩阵(current frame 2 到 reference
 * frame 1),并得到该模型的评分
 */
 ...
 * @brief 计算基础矩阵
 *
 * 假设场景为非平面情况下通过前两帧求取Fundamental矩阵(current frame 2 到
 * reference frame 1),并得到该模型的评分
 */
 ...
 * - Multiple View Geometry in Computer Vision - symmetric transfer
 * errors: 4.2.2 Geometric distance
...
float Initializer::CheckFundamental(const cv::Mat &F21,
  ...
  for (int i = 0; i < N; i++) {
    ...
    const float squareDist1 =
        num2 * num2 / (a2 * a2 + b2 * b2);  // 点到线的几何距离 的平方
...
bool Initializer::ReconstructF(vector<bool> &vbMatchesInliers, cv::Mat &F21,
  ...
  // initialization 四个结果中如果没有明显的最优结果，则返回失败
...
// decomposition 参考文献：Motion and structure from motion in a piecewise
// plannar environment 这篇参考文献和下面的代码使用了Faugeras SVD-based
// decomposition算法
 ...
 *
 * @see
 * - Faugeras et al, Motion and structure from motion in a piecewise planar
 ...
 * @param P2  投影矩阵P２
 * @param x3D 三维点
 * @see       Multiple View Geometry in Computer Vision - 12.2 Linear
 * triangulation methods p312
 */
